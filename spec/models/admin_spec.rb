    ...
    context '登録が上手くいかないとき' do
      ...
      it "emailが空だと登録できない" do
       ...
       it "passwordが空だと登録できない" do
        @admin.password=nil
        @admin.valid?
        expect(@admin.errors.full_messages).to include( "パスワードを入力してください", "パスワードは不正な値です", "パスワード（確認）とパスワードの入力が一致しません")
       end
       it "passwordが6文字以上でないと登録できない" do
        @admin.password ="aaa12"
        @admin.valid?
        expect(@admin.errors.full_messages).to include("パスワード（確認）とパスワードの入力が一致しません", "パスワードは6文字以上で入力してください", "パスワードは不正な値です")
       end
